...
def get_seg_model(cfg, **kwargs):
    model = HighResolutionNet(cfg, **kwargs).cuda()
    # output_list = model(torch.randn(1,3,256,256).cuda())
    # print([out.shape for out in output_list])
...
def main_OCR():
    ...
    print(model)
    input = torch.randn(10, 3, 256, 256).cuda()
    output = model(input)
